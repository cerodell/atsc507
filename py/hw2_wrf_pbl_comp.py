...
""" ############################# Make Plots ############################## """
%matplotlib
theTime = np.array2string(ds_base.rh2[4].Time)
...
C3 = f2_ax3.contourf(to_np(lon), to_np(lat),ds_ysu.rh2[0]- ds_base.rh2[0], extend = 'both',
                 transform=crs.PlateCarree(), levels = level, cmap=cmap)
...
C6 = f2_ax6.contourf(to_np(lon), to_np(lat), ds_ysu.T2[0] - ds_base.T2[0], extend = 'both',
                 transform=crs.PlateCarree(), levels = level, cmap=cmap)
