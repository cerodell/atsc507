...
# cord = getvar(str(filein)+'wrfout_d03_2019-08-19_00:00:00', "rh2")
shape = shape_list[0]
cord = cord_list[0]
...
for i in range(0,len(ffmc)):  
    ...
    v = np.linspace(70,100,61)
    Cnorm = colors.Normalize(vmin= np.min(ffmc), vmax =np.max(ffmc))


    C = plt.contourf(to_np(lons), to_np(lats),ffmc[i], extend = 'both',
                    transform=crs.PlateCarree(), norm = Cnorm, levels = v, cmap='jet')

    # clb = fig.colorbar(C,fraction=0.04, pad=0.02)
    # clb.set_label("$FFMC$", fontsize = 12)
